...
if (!empty($_GET['modulo']) && is_array($_GET['modulo'])) {
    $placeholders = implode(',', array_fill(0, count($_GET['modulo']), '?'));
    $condicion = "WHERE modulo IN ($placeholders)";
    $params_filtro = $_GET['modulo'];
...
$query_modulo = http_build_query(['modulo' => $_GET['modulo'] ?? []]);
    ...
    <div class="contenedor-bitacora">
                    ...
                    <?php foreach ($modulos_disponibles as $mod): ?>
                        <label class="form-filtro__label">
                            <input type="checkbox" name="modulo[]" value="<?= htmlspecialchars($mod) ?>"
                                <?= in_array($mod, $_GET['modulo'] ?? []) ? 'checked' : '' ?>>
                ...
                <tbody>
                    <?php if ($result && $result->num_rows > 0): ?>
                        <?php while ($row = $result->fetch_assoc()):
                            // Normalizamos la clase (quitar tildes y espacios)
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="5" class="tabla-bitacora__td">No se encontraron registros.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <?php if ($total_pag > 1): ?>
            <div class="paginacion">
                <?php if ($pagina > 1): ?>
                    <a class="paginacion__link" href="ver_bitacora.php?pagina=<?= ($pagina - 1) ?>&<?= htmlspecialchars($query_modulo) ?>">&laquo; Ant</a>
                <?php endif; ?>

                <?php for ($i = max(1, $pagina - 2); $i <= min($total_pag, $pagina + 2); $i++): ?>
                    <a href="ver_bitacora.php?pagina=<?= $i ?>&<?= htmlspecialchars($query_modulo) ?>"
                        class="paginacion__link <?= ($i === $pagina) ? 'paginacion__link--active' : '' ?>"><?= $i ?></a>
                <?php endfor; ?>

                <?php if ($pagina < $total_pag): ?>
                    <a class="paginacion__link" href="ver_bitacora.php?pagina=<?= ($pagina + 1) ?>&<?= htmlspecialchars($query_modulo) ?>">Sig &raquo;</a>
                <?php endif; ?>
            </div>
        <?php endif; ?>
    </div>
